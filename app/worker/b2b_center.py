"""B2B-Center market list + view.html cards (httpx + BeautifulSoup). No SOAP / Playwright."""
...
def cookies_path() -> Path:
    raw = os.getenv("B2B_CENTER_COOKIES_FILE", "./cookies.b2b-center.txt").strip()
    path = Path(raw)
...
def base_url() -> str:
    return (os.getenv("B2B_CENTER_BASE_URL") or DEFAULT_BASE).rstrip("/")
...
def parse_list_html(html: str, *, base: str | None = None) -> list[B2BCenterRow]:
    root = (base or base_url()).rstrip("/")
    soup = BeautifulSoup(html, "lxml")
    rows: list[B2BCenterRow] = []
    seen: set[str] = set()

    links = soup.select("table.search-results a.search-results-title")
    if not links:
        links = soup.select(
            'a[href*="view.html?id="], a[href*="/tender-"]'
        )
...
def parse_card_html(html: str, *, title_hint: str = "", base: str | None = None) -> dict[str, Any]:
    ...
    if soup.title and (not title or title in {"– B2B-Center", "- B2B-Center"}):
        page_title = soup.title.get_text(strip=True)
        if page_title and "B2B-Center" not in page_title.replace("—", "-"):
            title = page_title
        elif page_title:
            # "Title — Org — B2B-Center"
    ...
    return {
        ...
        "doc_links": doc_links,
        "card_fetched": True,
        "url": None,  # caller keeps canonical view.html
    }
...
def probe_b2b_center_session(
    ...
    *,
    ...
    """ok | missing | expired — market page with optional jar."""
    ...
    try:
        ...
        with httpx.Client(
            ...
            if "logout" in text or "личный кабинет" in text:
                return "ok"
            # File present and market loads — treat as ok (list is public).
            if response.status_code == 200 and "b2b-center" in text:
                return "ok"
            return "expired"
...
def scrape_list_page(
    *,
    ...
    try:
        ...
        if response.status_code in {401, 403}:
            raise AuthError("b2b_center_session_expired")
        response.raise_for_status()
        rows = parse_list_html(response.text, base=root)
...
def scrape_queries(
    *,
    ...
    """Union of f_keyword searches (trade=buy), deduped by native id."""
            ...
            while page <= MAX_PAGES and (cap is None or len(combined) < cap):
                ...
                for row in batch:
                    ...
                    if not tid or tid in seen:
                        continue
                    seen.add(tid)
                    row["url"] = card_url(tid, base=root)
                    combined.append(row)
                    new += 1
                    if cap is not None and len(combined) >= cap:
                        break
...
def enrich_cards(
    ...
    *,
    ...
    """Fetch view.html cards for L1–L3. Always rewrite URL to canonical view."""
    root = (base or base_url()).rstrip("/")
