...
7. **VPS:** [`docs/delivery/vps.md`](./docs/delivery/vps.md). Host `77.91.94.111`, key `tenders-ndt-vps`. Password in `.env.vps` — never print or commit. Do not rotate unless the owner asks. Password SSH stays on.
...
- VPS: do not disable password SSH; do not publish Scout login on public HTTP; do not treat owner-given VPS creds as compromised unless asked to rotate.
