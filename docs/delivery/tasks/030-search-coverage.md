...
- [x] Нет обязательного потолка 1000 в продукте/коде прогона
- [x] Сиды A–E на rostender; порядок очереди слож→прост→страховка
- [x] Tender.Pro пакеты + `in_queue` при cookies OK
- [x] `platforms.md` отражает 024 done
- [x] unit/smoke: scrape_queries без ранней обрезки по старому cap (или cap снят)
