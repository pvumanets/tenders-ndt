...
**порядок:** после [027](./027-expired-column.md), перед [029](./029-tier-rules-and-ai.md). **P9 done.**
...
- [x] Повторный прогон не создаёт вторую строку
- [x] Diff с площадкой → обновление полей; без diff → skip update
- [x] `viewed` / `manual_tier` / AI-статус не сбрасываются
- [x] В Tech видны **четыре** счётчика (полные фразы)
- [x] unit/smoke на already_exists / updated_from_platform
...
- Смена пула на `tier ∈ {L1,L2,L3}` (остаётся score≥4 до 029)
