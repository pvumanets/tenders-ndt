...
| Пул inbox / непросмотренные | **Lock 2026-08-27 / P12:** на доске **L1+L2+L3** (Горячие / Сильные / **Смотреть** — системой). Канон API: [`../delivery/sales-inbox-api.md`](../delivery/sales-inbox-api.md). **AS-IS runtime** до 029: score≥4 | fact |
...
| Документы | Имена **и файлы** на демо — **must** (`DOWNLOAD_DOCS` для лотов на доске L1–L3; AS-IS до 029: score≥4) | fact |
...
- [ ] Директор за ≤1 мин видит непросмотренные лоты пула L1–L3 **без** жаргона L1/L2/L3 (ярлыки Горячие / Сильные / Смотреть). AS-IS runtime до 029 может ещё фильтровать score≥4.  
...
```text
Done: P5.0–P7 + поиски (023) + Tender.Pro (024)
Now: следующий пункт бэклога владельца
```

Живой прогон Tender.Pro — человек в UI; cookies ЛК на VPS — `--sync` после merge.
